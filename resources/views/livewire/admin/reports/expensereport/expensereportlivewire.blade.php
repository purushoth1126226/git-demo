    ...
    <div class="card shadow-sm">
        <div class="card-header text-white theme_bg_color p-1">
            <div class="d-flex flex-row bd-highlight">
                <div class="flex-grow-1 bd-highlight mt-1"><span class="h5"> {{ __('EXPENSE REPORTS') }}</span>
                </div>
        ...
        <div class="card-body p-0">
            <div class="row g-2 align-items-center p-2">
                ...
                <div class="col-auto">
                    <select wire:model.lazy="activestatus" class="form-select form-select-sm ">
                        <option value="0">{{ __('Select Status') }} :</option>
                        @foreach ($this->expensecategory as $key => $value)
            ...
            <div class="table-responsive">
                <table id="appregistrationreport_id" class="table text-center table-hover m-0 p-0">
                    <thead>
                        <tr
                            ...
                            <th>{{ __('COMPANY / PERSON / SERVICE NAME') }} :</th>
                            <th>{{ __('AMOUNT') }} :</th>
                            <th>{{ __('EXPENSE DATE') }} :</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($this->expensereport as $key => $item)
                            <tr>
